    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if default_image_url:
                ...
                response = requests.get(default_image_url, stream=True, allow_redirects=True)
                with open(save_path, 'wb') as out_file:
                    for chunk in response.iter_content(1024):
                        out_file.write(chunk)
...
def create_product_with_images(n):
    for _ in range(n):
        ...
        category_image_path = os.path.join('media/photos/categories', category_image_name)
        os.makedirs(os.path.dirname(category_image_path), exist_ok=True)

        if not os.path.exists(category_image_path):
            download_unsplash_image(category_name.replace(' ', '+'), category_image_path)
        ...
        for variation_category, variation_list in variation_values.items():
            for variation_value in variation_list:
                ...
                gallery_image_name = f"{product_name.replace(' ', '_')}_{variation_value}_gallery.jpg"
                gallery_image_path = os.path.join('media/store/products', gallery_image_name)
                ...
                if not os.path.exists(gallery_image_path):
                    ...
                    variation_search_term = f"{product_name.replace(' ', '+')}+{variation_value}"
                    if not download_unsplash_image(variation_search_term, gallery_image_path):
                        # If variation-specific image fails, use base product image
                        download_unsplash_image(product_name.replace(' ', '+'), gallery_image_path)
        ...
        print(f'🎯 Created product: {product_name} in category: {category_name} with variations and gallery images.')
